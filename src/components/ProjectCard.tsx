    ...
    <div className="group relative flex flex-col rounded-none overflow-hidden glass-panel glass-panel-hover transition-all duration-300 border border-slate-800/80 hover:border-cyan-500/30" style={{ borderRadius: '0px' }}>
      ...
      <div className="relative h-52 w-full overflow-hidden bg-slate-950">
        ...
        <div className="absolute top-3 left-3 flex items-center gap-2">
          <span className={`px-2.5 py-1 rounded-none text-xs font-semibold uppercase tracking-wider font-mono border backdrop-blur-md ${
            is3rdYear 
          ...
          {project.featured && (
            <span className="px-2 py-0.5 rounded-none text-[11px] font-medium bg-amber-500/20 text-amber-300 border border-amber-500/30 backdrop-blur-md flex items-center gap-1">
        ...
        <div className="absolute bottom-3 left-3">
          <span className="text-[11px] font-mono px-2 py-0.5 rounded-none bg-slate-900/80 text-slate-300 border border-slate-700/60 backdrop-blur-md">
            {project.category}
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 p-5 sm:p-6 flex flex-col justify-between space-y-4">
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {project.techStack.slice(0, 4).map((tech, idx) => (
            <span 
              key={idx} 
              className="text-[11px] font-mono px-2 py-0.5 rounded-none bg-slate-800/80 text-slate-300 border border-slate-700/50"
            >
              {tech}
            </span>
          ))}
          {project.techStack.length > 4 && (
            <span className="text-[11px] font-mono px-1.5 py-0.5 rounded-none bg-slate-800/40 text-slate-400">
              +{project.techStack.length - 4}
            </span>
          )}
        </div>
        ...
        <div className="pt-2 flex items-center justify-between gap-3">
          <a
            ...
            className="p-2 rounded-none bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-white border border-slate-800 hover:border-slate-700 transition-colors"
          >
