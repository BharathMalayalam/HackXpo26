...
export const Projects2ndYearPage: React.FC = () => {
  ...
  const secondYearProjects = useMemo(() => {
    return projectsData.filter((p) => p.year === '2nd Year');
  }, []);

  const filteredProjects = useMemo(() => {
    return secondYearProjects.filter((p) => {
  ...
  }, [secondYearProjects, selectedTech, searchQuery]);
    ...
    <div className="min-h-screen bg-[#06080F] text-slate-100 py-12 tech-grid-bg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="space-y-4 text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-none bg-purple-500/10 text-purple-400 border border-purple-500/20 text-xs font-mono">
            <FolderGit2 className="w-3.5 h-3.5" />
            <span>2nd Year Cohort</span>
          </div>
          ...
          <p className="text-sm sm:text-base text-slate-300 font-light max-w-2xl mx-auto">
            Full-stack rapid prototypes, accessibility tools, and campus digital solutions engineered by <strong>2nd Year</strong> students.
          </p>
        </div>
        ...
        <div className="p-6 rounded-none bg-[#0A0E17]/90 border border-slate-800 backdrop-blur-md space-y-6 shadow-xl" style={{ borderRadius: '0px' }}>
          ...
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-3 bg-slate-900/90 border border-slate-800 rounded-none text-sm text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 transition-colors"
            ...
            <div className="flex items-center gap-3 shrink-0">
              <span className="text-xs font-mono px-3 py-1.5 rounded-none bg-purple-500/10 text-purple-300 border border-purple-500/20" style={{ borderRadius: '0px' }}>
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1">
            <span className="text-xs font-mono text-slate-400 shrink-0 flex items-center gap-1">
              <Code2 className="w-3.5 h-3.5 text-purple-400" /> Tech:
            </span>
        ...
        {filteredProjects.length === 0 ? (
          <div className="text-center py-20 bg-slate-900/40 rounded-none border border-slate-800 space-y-4" style={{ borderRadius: '0px' }}>
            ...
            <button
              onClick={resetFilters}
              className="px-5 py-2.5 rounded-none bg-slate-800 hover:bg-slate-700 text-purple-400 text-xs font-semibold transition-colors"
              style={{ borderRadius: '0px' }}
            >
              Reset Filters
            </button>
